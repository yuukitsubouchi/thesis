...
\begin{figure}[H]
 ...
 \includegraphics[keepaspectratio, scale=0.3]
      {images/117.png}
 \caption{Overview of ORNE-box3 and its onboard sensors(source: \cite{井口颯人2023屋外自律移動ロボットプラットフォーム-orne})}
 \label{Fig:Overview of ORNE-box3 and its onboard sensors}
\end{figure}

実験には，本研究室で開発されている自律移動ロボット ORNE-box3\cite{井口颯人2023屋外自律移動ロボットプラットフォーム-orne} を使用した．ORNE-box3 の外観を\figref{Fig:Overview of ORNE-box3 and its onboard sensors} に示す．
...
3D LiDAR は自己位置推定および障害物検知に使用され，IMU とエンコーダから得られる情報は自己位置推定ノードの emcl2 に入力され，ロボットの自己位置推定に利用される．
...
各走行実験においては，対象とするパラメータのみを変更し，それ以外のパラメータは実験前に設定していた値を基準値として固定した. これにより，特定のパラメータ変更がロボットの挙動に与える影響を明確にすることを試みた．
...
ゴール到達の可否を評価指標として用いた．実験回数は,各条件につき10回実施とした.

\subsubsection{実験2}
num\_particles以外のパラメータについては，自己位置推定の結果を用いて, 基準値に対するロボットの位置と各条件におけるロボットの位置を比較した．実験回数は, 各条件につき 1 回の走行実験を実施とした. 
...
また，変更したパラメータの対応をTable \ref{tab:exp_emcl2}〜Table \ref{tab:exp_planner_goal}に示す．各実験では，表に示したパラメータのみを基準値から変更し，その他の設定はすべて基準値と同一とした．
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \hline
    Number of particles & 200, 1000 \\
    odom\_fw\_dev\_per\_fw & 0.01, 0.05, 0.1, 0.3, 0.4, 0.5 \\
    odom\_fw\_dev\_per\_rot & 0.001, 0.01, 0.1, 0.5 \\
    odom\_rot\_dev\_per\_fw & 0.01, 0.2, 0.3, 0.4, 0.5 \\
    odom\_rot\_dev\_per\_rot & 0.1, 0.3, 0.4, 0.5 \\
    laser\_likelihood\_max\_dist & 0.5, 1.0 \\
    range\_threshold & 0.5, 0.9 \\
...
%     Experiment ID & Parameter & Tested Values \\
    ...
    \hline
    max\_vel\_x & 0.2, 1.0 \\
    max\_vel\_theta & 0.4, 1.0 \\
    max\_speed\_xy & 0.1, 0.5, 1.5 \\
    min\_theta\_velocity\_threshold & 0.01, 0.1, 1.0 \\
    acc\_lim\_x & 0.1, 0.5, 1.5 \\
    acc\_lim\_theta & 0.1, 1.0, 4.0 \\
    linear\_granularity & 0.01, 0.1, 0.5 \\
    angular\_granularity & 0.001, 0.01, 0.04 \\
    xy\_goal\_tolerance & 0.01, 0.1, 0.4 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    ...
    Scope & Parameter & Tested Values \\
    \hline
    Global & global\_resolution & 0.05, 0.5, 1.0 \\
    Global & global\_cost\_scaling\_factor & 1.0, 5.0, 10.0 \\
    Global & global\_inflation\_radius & 0.1, 1.0, 2.0 \\
    Local  & local\_resolution & 0.05, 0.5, 1.0 \\
    Local  & local\_cost\_scaling\_factor & 1.0, 5.0, 10.0 \\
    Local  & local\_inflation\_radius & 0.1, 0.5, 1.0, 2.0 \\
...
%     Experiment ID & Parameter & Tested Values \\
    \hline
    smoothing\_frequency & 5, 10, 40, 60 \\
    max\_velocity\_x & 0.3, 0.9, 1.2 \\
    max\_velocity\_theta & 0.6, 2.5 \\
    max\_accel\_x & 0.2, 1.0 \\
    max\_accel\_theta & 0.5, 2.0 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Parameter & Tested Values \\
    \hline
    trans\_stopped\_velocity & 0.01, 0.1, 0.4 \\
    planner\_tolerance & 0.1, 1.0 \\
...
%\end{figure}
\subsection{実験結果}
\subsubsection{実験1結果(num\_particles)}
基準となるパーティクル数は500である.
\begin{table}[H]
  \centering
...
\section{実験2の結果}
次からの実験では, ロボットの位置を比較する. \figref{Fig:Trajectory of the robot controlled by baseline parameters}のグラフが基準の値の時のロボットの位置である. このロボットの位置のグラフにおけるゴールは, x座標-275～-270, y座標-380～-370の所である.また,スタート地点を基準にしているため\figref{Fig:Trajectory of the robot controlled by baseline parameters}のようなグラフになっている.
% \begin{figure}[H]
%   \centering
%  \includegraphics[keepaspectratio, scale=0.6]
%       {images/1.png}
...
\begin{figure}[H]
 ...
 \includegraphics[keepaspectratio, scale=0.6]
      {images/base111.png}
 \caption{Trajectory of the robot controlled by baseline parameters}
 \label{Fig:Trajectory of the robot controlled by baseline parameters}
...
%ロボットの位置と向きは、\figref{Fig:Base orientation}, \figref{Fig:Base position}で示す
% \begin{figure}[H]
%   \centering
%  \includegraphics[keepaspectratio, scale=0.6]
...
\begin{figure}[H]
 ...
 \caption{Trajectory of the robot with fw\_dev\_per\_fw = 0.01
 %\caption{Robot position with fw\_dev\_per\_fw = 0.01
}
 \label{Fig:Trajectory of the robot with fw_dev_per_fw = 0.01}
\end{figure}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwfw0.05iti3.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_fw = 0.05
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwfw0.1iti3.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_fw = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwfw0.3iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_fw = 0.3
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwfw0.4iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_fw = 0.4
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwfw0.5iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_fw = 0.5
}
 \label{Fig:Robot position with fw_dev_per_fw = 0.5}
\end{figure}

\figref{Fig:Robot position with fw_dev_per_fw = 0.5}で示すように、odom\_fw\_dev\_per\_fw を 0.5 に設定した場合，ロボットは走行を続けるにつれて自己位置推定の誤差が累積し，最終的にゴールに到達することができなかった．一方で，0.4 以下の値に設定した場合には，ロボットはゴールまで到達することが確認された．
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwrot0.001iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_rot = 0.001
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwrot0.01iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_rot = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwrot0.1iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_rot = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/fwrot0.5iti2.png}
 \caption{Trajectory of the robot with fw\_dev\_per\_rot = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotfw0.01iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_fw = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotfw0.2iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_fw = 0.2
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotfw0.3iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_fw = 0.3
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotfw0.4iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_fw = 0.4
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotfw0.5iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_fw = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotrot0.1iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_rot = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotrot0.3iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_rot = 0.3
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotrot0.4iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_rot = 0.4
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rotrot0.5iti2.png}
 \caption{Trajectory of the robot with rot\_dev\_per\_rot = 0.5
}
...
本実験の結果，rot\_dev\_per\_rot の値を大きく設定した場合(0.4,0.5)，
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/llmd0.5iti2.png}
 \caption{Trajectory of the robot with laser\_likelihood\_max\_dist = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/llmd1.0iti2.png}
 \caption{Trajectory of the robot with laser\_likelihood\_max\_dist = 1.0
}
...
3DLidarのレーザによって評価される情報量が増加し，
特に障害物や構造物が多く存在する環境において，
自己位置推定の更新が頻繁に行われていたためと考えられる.
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rt0.5iti2.png}
 \caption{Trajectory of the robot with range\_threshold = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/rt0.9iti2.png}
 \caption{Trajectory of the robot with range\_threshold = 0.9
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvx0.2iti2.png}
 \caption{Trajectory of the robot with max\_vel\_x = 0.2
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvx1.0iti2.png}
 \caption{Trajectory of the robot with max\_vel\_x = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvt0.4iti2.png}
 \caption{Trajectory of the robot with max\_vel\_theta = 0.4
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvt1.0iti2.png}
 \caption{Trajectory of the robot with max\_vel\_theta = 1.0
}
...
ロボットの挙動が細かく振動的になっている様子がグラフより確認された．
特に，旋回が必要となる箇所において，
ロボットが一時的に停止し，
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/msxy0.1iti2.png}
 \caption{Trajectory of the robot with max\_speed\_xy = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/msxy0.5iti2.png}
 \caption{Trajectory of the robot with max\_speed\_xy = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mtvt0.01iti2.png}
 \caption{Trajectory of the robot with min\_theta\_velocity\_threshold = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mtvt0.1iti2.png}
 \caption{Trajectory of the robot with min\_theta\_velocity\_threshold = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mtvt1.0iti2.png}
 \caption{Trajectory of the robot with min\_theta\_velocity\_threshold = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/alx0.1iti2.png}
 \caption{Trajectory of the robot with acc\_lim\_x = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/alx0.5iti2.png}
 \caption{Trajectory of the robot with acc\_lim\_x = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/alx1.5iti2.png}
 \caption{Trajectory of the robot with acc\_lim\_x = 1.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/alt0.1iti2.png}
 \caption{Trajectory of the robot with acc\_lim\_theta = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/alt1.0iti2.png}
 \caption{Trajectory of the robot with acc\_lim\_theta = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/alt4.0iti2.png}
 \caption{Trajectory of the robot with acc\_lim\_theta = 4.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lg0.01iti2.png}
 \caption{Trajectory of the robot with linear\_granularity = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lg0.1iti2.png}
 \caption{Trajectory of the robot with linear\_granularity = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lg0.5iti2.png}
 \caption{Trajectory of the robot with linear\_granularity = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/ag0.001iti2.png}
 \caption{Trajectory of the robot with angular\_granularity = 0.001
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/ag0.01iti2.png}
 \caption{Trajectory of the robot with angular\_granularity = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/ag0.04iti2.png}
 \caption{Trajectory of the robot with angular\_granularity = 0.04
}
...
一定範囲内で値を小さく設定しても走行挙動に大きな影響を与えないことが示された.
% 走行挙動に大きな影響を与えない一方で，
% 過度に大きな値を設定した場合には，
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/xygt0.01iti2.png}
 \caption{Trajectory of the robot with xy\_goal\_tolerance = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/xygt0.1iti2.png}
 \caption{Trajectory of the robot with xy\_goal\_tolerance = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/xygt0.4iti2.png}
 \caption{Trajectory of the robot with xy\_goal\_tolerance = 0.4
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gr0.05iti2.png}
 \caption{Trajectory of the robot with global\_resolution = 0.05
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gr0.5iti2.png}
 \caption{Trajectory of the robot with global\_resolution = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gr1.0iti2.png}
 \caption{Trajectory of the robot with global\_resolution = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gcsf1.0iti2.png}
 \caption{Trajectory of the robot with global\_cost\_scaling\_factor = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gcsf5.0iti2.png}
 \caption{Trajectory of the robot with global\_cost\_scaling\_factor = 5.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gcsf10iti2.png}
 \caption{Trajectory of the robot with global\_cost\_scaling\_factor = 10
}
...
そのため，実験結果では,あまり変化が見られなかった.
このパラメータは,環境中の障害物配置や安全マージンの要求に応じて，
適切な値を選択することが重要であると考えられる.
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gir0.1iti2.png}
 \caption{Trajectory of the robot with global\_inflation\_radius = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gir1.0iti2.png}
 \caption{Trajectory of the robot with global\_inflation\_radius = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/gir2.0iti2.png}
 \caption{Trajectory of the robot with global\_inflation\_radius = 2.0
}
...
その結果，障害物や建物に反応して細かな動きがグラフから読み取れる.
% 道幅が狭い最初の直線区間において，
% ロボットが通行可能な領域が制限され，
% 進行開始がやや遅れる挙動が確認された．
しかしながら，
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lr0.05iti2.png}
 \caption{Trajectory of the robot with local\_resolution = 0.05
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lr0.5iti2.png}
 \caption{Trajectory of the robot with local\_resolution = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lr1.0iti2.png}
 \caption{Trajectory of the robot with local\_resolution = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lcsf1.0iti2.png}
 \caption{Trajectory of the robot with local\_cost\_scaling\_factor = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lcsf5.0iti2.png}
 \caption{Trajectory of the robot with local\_cost\_scaling\_factor = 5.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lcsf10iti2.png}
 \caption{Trajectory of the robot with local\_cost\_scaling\_factor = 10
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lir0.1iti2.png}
 \caption{Trajectory of the robot with local\_inflation\_radius = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lir0.5iti2.png}
 \caption{Trajectory of the robot with local\_inflation\_radius = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lir1.0iti2.png}
 \caption{Trajectory of the robot with local\_inflation\_radius = 1.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/lir2.0iti2.png}
 \caption{Trajectory of the robot with local\_inflation\_radius = 2.0
}
...
\begin{figure}[H]
 ...
 \includegraphics[keepaspectratio, scale=0.6]
      {images/sf5iti2.png}
 \caption{Trajectory of the robot with smoothing\_frequency = 5
}
...
\begin{figure}[H]
 ...
 \includegraphics[keepaspectratio, scale=0.6]
      {images/sf10iti2.png}
 \caption{Trajectory of the robot with smoothing\_frequency = 10
}
...
\begin{figure}[H]
 ...
 \includegraphics[keepaspectratio, scale=0.6]
      {images/sf40iti2.png}
 \caption{Trajectory of the robot with smoothing\_frequency = 40
}
...
\begin{figure}[H]
 ...
 \includegraphics[keepaspectratio, scale=0.6]
      {images/sf60iti2.png}
 \caption{Trajectory of the robot with smoothing\_frequency = 60
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvx0.3iti2.png}
 \caption{Trajectory of the robot with max\_velocity\_x = 0.3
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvx0.9iti2.png}
 \caption{Trajectory of the robot with max\_velocity\_x = 0.9
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvx1.2iti2.png}
 \caption{Trajectory of the robot with max\_velocity\_x = 1.2
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvt0.6iti2.png}
 \caption{Trajectory of the robot with max\_velocity\_theta = 0.6
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mvt2.5iti2.png}
 \caption{Trajectory of the robot with max\_velocity\_theta = 2.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mams0.2iti2.png}
 \caption{Trajectory of the robot with max\_accel\_x = 0.2
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mams1.0iti2.png}
 \caption{Trajectory of the robot with max\_accel\_x = 1.0
}
...
最初の曲がり角において一時的に停止する挙動が確認されたり，少し振動的な追従挙動があったが,
全体的に安定した走行を維持し，
最終的にはゴールに到達した．
この結果から，
加速度を大きく設定した場合でも，
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mars0.5iti2.png}
 \caption{Trajectory of the robot with max\_accel\_theta = 0.5
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/mars2.0iti2.png}
 \caption{Trajectory of the robot with max\_accel\_theta = 2.0
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/tsv0.01iti2.png}
 \caption{Trajectory of the robot with trans\_stopped\_velocity = 0.01
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/tsv0.1iti2.png}
 \caption{Trajectory of the robot with trans\_stopped\_velocity = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/tsv0.4iti2.png}
 \caption{Trajectory of the robot with trans\_stopped\_velocity = 0.4
}
...
Waypoint からやや離れた位置で行われるため,グラフでの最後の曲がるところで少し大回りをしていた．
...
しかし, 0.1の時は最後の曲がるところで振動的な追従走行になっていた.
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/pt0.1iti2.png}
 \caption{Trajectory of the robot with planner\_tolerance = 0.1
}
...
\begin{figure}[H]
  \centering
 \includegraphics[keepaspectratio, scale=0.6]
      {images/pt1.0iti2.png}
 \caption{Trajectory of the robot with planner\_tolerance = 1.0
}
...
その結果，ロボットの走行挙動に大きな影響を与えるものではなく，
